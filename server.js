...
const HOST = process.env.HOST || '0.0.0.0'; // importante para containers
...
async function startServer() {
  try {
    ...
    // Iniciar servidor HTTP (escutando no PORT e em 0.0.0.0)
    server = app.listen(PORT, HOST, () => {
      console.log('🎉 Servidor iniciado com sucesso!');
      console.log(`🌐 URL: ${process.env.BACKEND_URL || '(defina BACKEND_URL)'}`);
...
function checkEnvironmentVariables() {
  ...
  if (missingVars.length > 0) {
    ...
    console.error('\n💡 Verifique seu arquivo .env ou as Environment Variables no Coolify');
    process.exit(1);
  }
}
